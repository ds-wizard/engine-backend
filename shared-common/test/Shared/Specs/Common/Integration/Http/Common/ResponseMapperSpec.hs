module Shared.Specs.Common.Integration.Http.Common.ResponseMapperSpec where
        ...
        do
          ...
          let expectation =
                Left . GeneralServerError $
                  _ERROR_INTEGRATION_COMMON__UNABLE_TO_EXTRACT_NESTED_FIELDS ["nonExistingField"]
          -- WHEN:
    ...
    describe "extractStringField" $ do
      ...
      it "non-existing field" $
        ...
        do
          ...
          let expectation =
                Left . GeneralServerError $
                  _ERROR_INTEGRATION_COMMON__UNABLE_TO_EXTRACT_STRING_FIELD "nonExistingField"
          -- WHEN:
    ...
    describe "convertToArray" $ do
      ...
      it "bad type" $
        ...
        do
          let response = obj [("targetProp", String "Target Content")]
          -- AND: Expectations
          let expectation = Left . GeneralServerError $ _ERROR_INTEGRATION_COMMON__FIELD_IS_NOT_ARRAY
          -- WHEN:
          let result = convertToArray response
          -- THEN:
          result `shouldBe` expectation
