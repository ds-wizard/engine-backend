...
runInUnauthService :: TransactionState -> AppContextM a -> BaseContextM a
runInUnauthService = runIn Nothing

runInAuthService :: Organization -> TransactionState -> AppContextM a -> BaseContextM a
runInAuthService org = runIn (Just org)

runIn :: Maybe Organization -> TransactionState -> AppContextM a -> BaseContextM a
    ...
    case transactionState of
      Transactional -> do
        liftIO $ withResource dbPool $ \dbConn -> do
          ...
          liftIO $ runExceptT $ runLogging loggingLevel $ runReaderT (runAppContextM function) appContextWithConn
      NoTransaction -> liftIO $ runExceptT $ runLogging loggingLevel $ runReaderT (runAppContextM function) appContext
  case eResult of
    Right result -> return result
    Left error -> throwError =<< sendError error

getMaybeAuthServiceExecutor ::
     Maybe String -> ((TransactionState -> AppContextM a -> BaseContextM a) -> BaseContextM b) -> BaseContextM b
