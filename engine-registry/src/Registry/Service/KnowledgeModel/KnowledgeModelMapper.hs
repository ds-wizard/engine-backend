module Registry.Service.KnowledgeModel.KnowledgeModelMapper where
...
fromMetricMeasureDTO m =
  MetricMeasure
    { _metricMeasureMetricUuid = m ^. metricUuid
    , _metricMeasureMeasure = m ^. measure
    , _metricMeasureWeight = m ^. weight
    }
