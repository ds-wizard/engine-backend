    ...
    describe "Situations (Core <?> Localization)" $ do
      ...
      it "Situation n.2: Edit < Edit (Corrector)" $
        ...
        do
          ...
          let expState =
                reqState {migrationState = ConflictState . CorrectorConflict . Just . EditQuestionEvent' . EditValueQuestionEvent' $ expEvent}
          resState `shouldBe` expState
      -- -------------------------------------------------------------
      -- -------------------------------------------------------------
      ...
      it "Situation n.9: Edit > Delete (Corrector)" $
        ...
        do
          ...
          let expEvent = e_km1_ch1 {uuid = resEvent.uuid, questionUuids = expEventEntityUuids} :: EditChapterEvent
          let expState =
                reqState
