  ...
  describe "MoveSanitizer" $ do
    describe "generateEvents" $
      it "Succeed" $
        -- GIVEN:
        do
          let (Right oldKm) = compile Nothing germanyEvents
          let moveEvents =
          ...
          let (Right newKm) = compile Nothing (germanyEvents ++ moveEvents)
          let expected =
                [ m_km1_ch1_q1__to_ch2 {uuid = U.nil} :: KnowledgeModelEvent
                , m_km1_ch2_q4_it1_q5__to_ch2_q4_it1_q6_aNo {uuid = U.nil} :: KnowledgeModelEvent
                , m_km1_ch2_q4_it1_q6_aYes_fuq4_it_q1__to_ch2_q4 {uuid = U.nil} :: KnowledgeModelEvent
                , m_km1_ch1_q2_aYes__to_ch2_q3 {uuid = U.nil} :: KnowledgeModelEvent
                ]
          -- WHEN:
