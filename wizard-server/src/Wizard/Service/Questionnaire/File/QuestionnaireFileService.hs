...
getQuestionnaireFilesPage mQuery mQtnUuid pageable sort = do
  checkPermission _QTN_FILE_PERM
  findQuestionnaireFilesPage mQuery mQtnUuid pageable sort

createQuestionnaireFile :: U.UUID -> FileCreateDTO -> AppContextM QuestionnaireFileList
createQuestionnaireFile qtnUuid reqDto =
  runInTransaction $ do
...
downloadQuestionnaireFile qtnUuid fileUuid = do
  runInTransaction $ do
    ...
    content <- retrieveFile qtnUuid fileUuid
    url <- createTemporaryFile qtnFile.fileName "application/octet-stream" (BSL.fromStrict content)
