module Wizard.Service.Migration.KnowledgeModel.Migrator.Sanitizer where
...
-- ------------------------------------------------------------
class Sanitizer a where
  sanitize :: MigratorState -> a -> AppContextM a

-- ------------------------------------------------------------
instance Sanitizer EditKnowledgeModelEvent where
...
-- ------------------------------------------------------------
instance Sanitizer EditChapterEvent where
  sanitize state event =
...
-- ------------------------------------------------------------
instance Sanitizer EditQuestionEvent where
  sanitize state event =
    unwrapKM state event $ \km -> do
...
-- ------------------------------------------------------------
instance Sanitizer EditAnswerEvent where
  sanitize state event =
    unwrapKM state event $ \km ->
      unwrapEventChildUuids $ \childUuidsFromEvent -> do
