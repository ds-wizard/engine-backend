...
createTables = do
  ...
  let sql =
        ...
        \    CONSTRAINT persistent_command_pk PRIMARY KEY (uuid, tenant_uuid), \
        \    CONSTRAINT persistent_command_created_by_fk FOREIGN KEY (created_by, tenant_uuid) REFERENCES user_entity (uuid, tenant_uuid), \
        \    CONSTRAINT persistent_command_tenant_uuid_fk FOREIGN KEY (tenant_uuid) REFERENCES tenant (uuid) \
        \);"
  let action conn = execute_ conn sql
  runDB action
