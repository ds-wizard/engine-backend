...
migrate dbPool = do
  ...
  makeVersionUuidUnique dbPool
  createVersionTable dbPool
  insertVersions dbPool
  addDocumentQuestionnaireUuidForeignKey dbPool
  addDocumentQuestionnaireEventUuidForeignKey dbPool
...
addDocumentQuestionnaireUuidForeignKey dbPool = do
  let sql = "ALTER TABLE document ADD CONSTRAINT document_questionnaire_uuid_fk FOREIGN KEY (questionnaire_uuid, tenant_uuid) REFERENCES questionnaire (uuid, tenant_uuid)"
  let action conn = execute_ conn sql
  liftIO $ withResource dbPool action
  return Nothing

addDocumentQuestionnaireEventUuidForeignKey dbPool = do
  let sql = "ALTER TABLE document ADD CONSTRAINT document_questionnaire_event_uuid_fk FOREIGN KEY (questionnaire_event_uuid, tenant_uuid) REFERENCES questionnaire_event (uuid, tenant_uuid) ON DELETE SET NULL;"
  let action conn = execute_ conn sql
  liftIO $ withResource dbPool action
  return Nothing
