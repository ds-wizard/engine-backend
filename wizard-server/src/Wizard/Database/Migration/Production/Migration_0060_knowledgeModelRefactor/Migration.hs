...
migrate dbPool = do
  ...
  -- CASCADE
  createPersistentCommandFromDocumentTemplateAssetDeleteFunction dbPool
  createDocumentTemplateAssetTrigger dbPool
  deleteDocumentWithoutQuestionnaire dbPool
  removeTenantUuidFromPrimaryKey dbPool
  createUnsetUserLocaleFunction dbPool
  createLocaleTriggers dbPool
  dropDocumentQuestionnaireNotNull dbPool
...
-- ------------------------------------------------------------------------------------------------------------------------
-- 6.1) CREATE PERSISTENT COMMAND FROM DOCUMENT TEMPLATE ASSET DELETE FUNCTION
-- ------------------------------------------------------------------------------------------------------------------------
createPersistentCommandFromDocumentTemplateAssetDeleteFunction dbPool = do
  let sql =
...
createDocumentTemplateAssetTrigger dbPool = do
  let sql =
...
deleteDocumentWithoutQuestionnaire dbPool = do
  let sql = "DELETE FROM document WHERE uuid in (SELECT document.uuid FROM document LEFT JOIN questionnaire ON questionnaire.uuid = document.questionnaire_uuid AND questionnaire.tenant_uuid = document.tenant_uuid WHERE questionnaire.uuid IS NULL);"
  let action conn = execute_ conn sql
  liftIO $ withResource dbPool action
  return Nothing
...
removeTenantUuidFromPrimaryKey dbPool = do
  let sql =
        ...
        \ALTER TABLE document ADD CONSTRAINT document_pk PRIMARY KEY (uuid); \
        \ALTER TABLE external_link_usage ADD CONSTRAINT external_link_usage_pk PRIMARY KEY (uuid); \
        \ALTER TABLE feedback ADD CONSTRAINT feedback_pk PRIMARY KEY (uuid); \
        \ALTER TABLE knowledge_model_editor ADD CONSTRAINT knowledge_model_editor_pk PRIMARY KEY (uuid); \
        \ALTER TABLE knowledge_model_editor_event ADD CONSTRAINT knowledge_model_editor_event_pk PRIMARY KEY (uuid); \
        \ALTER TABLE knowledge_model_editor_reply ADD CONSTRAINT knowledge_model_editor_reply_pk PRIMARY KEY (editor_uuid, path); \
        \ALTER TABLE knowledge_model_migration ADD CONSTRAINT knowledge_model_migration_pk PRIMARY KEY (editor_uuid); \
        \ALTER TABLE knowledge_model_secret ADD CONSTRAINT knowledge_model_secret_pk PRIMARY KEY (uuid); \
        \ALTER TABLE persistent_command ADD CONSTRAINT persistent_command_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire ADD CONSTRAINT questionnaire_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire_event ADD CONSTRAINT questionnaire_event_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire_perm_user ADD CONSTRAINT questionnaire_perm_user_pk PRIMARY KEY (questionnaire_uuid, user_uuid); \
        \ALTER TABLE questionnaire_perm_group ADD CONSTRAINT questionnaire_perm_group_pk PRIMARY KEY (questionnaire_uuid, user_group_uuid); \
        \ALTER TABLE questionnaire_comment_thread ADD CONSTRAINT questionnaire_comment_thread_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire_comment ADD CONSTRAINT questionnaire_comment_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire_version ADD CONSTRAINT questionnaire_version_pk PRIMARY KEY (uuid); \
        \ALTER TABLE questionnaire_file ADD CONSTRAINT questionnaire_file_pk PRIMARY KEY (uuid); \
        \ALTER TABLE submission ADD CONSTRAINT submission_pk PRIMARY KEY (uuid); \
        \ALTER TABLE temporary_file ADD CONSTRAINT temporary_file_pk PRIMARY KEY (uuid); \
        \ALTER TABLE user_entity ADD CONSTRAINT user_entity_pk PRIMARY KEY (uuid); \
        \ALTER TABLE user_entity_submission_prop ADD CONSTRAINT user_entity_submission_prop_pk PRIMARY KEY (user_uuid, service_id); \
        \ALTER TABLE user_token ADD CONSTRAINT user_token_pk PRIMARY KEY (uuid); \
        \ALTER TABLE user_group ADD CONSTRAINT user_group_pk PRIMARY KEY (uuid); \
...
createUnsetUserLocaleFunction dbPool = do
  let sql =
...
createLocaleTriggers dbPool = do
  let sql =
        "DROP TRIGGER trigger_on_after_locale_delete ON locale; \
...
dropDocumentQuestionnaireNotNull dbPool = do
  let sql = "ALTER TABLE document ALTER COLUMN questionnaire_uuid DROP NOT NULL;"
  let action conn = execute_ conn sql
  liftIO $ withResource dbPool action
  return Nothing
