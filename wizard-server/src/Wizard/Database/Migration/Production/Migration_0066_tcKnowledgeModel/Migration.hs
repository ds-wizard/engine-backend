...
upgradeDtMetamodel dbPool = do
  let sql = "UPDATE document_template SET metamodel_version = (18,0) WHERE phase = 'DraftDocumentTemplatePhase';"
  let action conn = execute_ conn sql
  liftIO $ withResource dbPool action
  return Nothing
