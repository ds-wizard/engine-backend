...
createTcAuthenticationOpenIdTable dbPool = do
  let sql =
        ...
        \); \
        \INSERT INTO config_authentication_openid (id, name, url, client_id, client_secret, parameteres, style, tenant_uuid, \
        ...
        \       (nested.json_content ->> 'style')::jsonb, \
        \       nested.tenant_uuid, \
        \       nested.created_at, \
        \       nested.updated_at \
...
createTcLookAndFeelTable dbPool = do
  let sql =
        ...
        \       (look_and_feel ->> 'illustrationColor')::varchar, \
        \       created_at, \
        \       updated_at \
        \FROM tenant_config;;"
  let action conn = execute_ conn sql
...
createTcKnowledgeModelPublicPackagePatternTable dbPool = do
  let sql =
        ...
        \    created_at      timestamptz NOT NULL, \
        \    updated_at      timestamptz NOT NULL, \
        \    CONSTRAINT config_knowledge_model_public_package_pattern_pk PRIMARY KEY (tenant_uuid), \
