...
updateQuestionnaireCommentThreadAssignedTo :: U.UUID -> Maybe U.UUID -> AppContextM Int64
updateQuestionnaireCommentThreadAssignedTo uuid assignedTo = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE questionnaire_comment_thread SET assigned_to = ?, updated_at = now() WHERE uuid = ? AND tenant_uuid = ?"
  let params = [toField assignedTo, toField uuid, toField tenantUuid]
  logQuery sql params
  let action conn = execute conn sql params
  runDB action
...
clearQuestionnaireCommentThreadAssignedTo userUuid = do
  let sql = fromString "UPDATE questionnaire_comment_thread SET assigned_to = null WHERE assigned_to = ?"
