...
findQuestionnaireFilesByQuestionnaire :: U.UUID -> AppContextM [QuestionnaireFileList]
findQuestionnaireFilesByQuestionnaire qtnUuid = do
  tenantUuid <- asks currentTenantUuid
  createFindEntitiesByFn entityName [tenantQueryUuid tenantUuid, ("questionnaire_uuid", U.toString qtnUuid)]
