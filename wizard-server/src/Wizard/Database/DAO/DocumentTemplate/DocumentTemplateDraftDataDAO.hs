...
unsetKnowledgeModelEditorFromDocumentTemplate :: U.UUID -> AppContextM Int64
unsetKnowledgeModelEditorFromDocumentTemplate qtnUuid = do
  tenantUuid <- asks currentTenantUuid
  let sql =
        fromString
          "UPDATE document_template_draft_data SET knowledge_model_editor_uuid = null, format_uuid = null WHERE tenant_uuid = ? AND knowledge_model_editor_uuid = ?"
  let params = [toField tenantUuid, toField qtnUuid]
  logQuery sql params
  let action conn = execute conn sql params
  runDB action
