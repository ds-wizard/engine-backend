...
updateTenantConfig config = do
  let sql =
        fromString
          "UPDATE tenant_config SET uuid = ?, organization = ?, authentication = ?, privacy_and_support = ?, dashboard_and_login_screen = ?, look_and_feel = ?, registry = ?, knowledge_model = ?, questionnaire = ?, created_at = ?, updated_at = ?, owl = ?, mail_config_uuid = ?, ai_assistant = ? WHERE uuid = ?"
