...
updateTenantByUuid tenant = do
  ...
  let sql =
        fromString
          "UPDATE tenant SET uuid = ?, tenant_id = ?, name = ?, server_domain = ?, client_url = ?, enabled = ?, created_at = ?, updated_at = ?, server_url = ?, admin_server_url = ?, admin_client_url = ?, integration_hub_server_url = ?, integration_hub_client_url = ? WHERE uuid = ?"
