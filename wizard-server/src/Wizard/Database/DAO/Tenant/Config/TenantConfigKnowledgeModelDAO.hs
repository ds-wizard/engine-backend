...
findTenantConfigKnowledgeModelByUuid tenantUuid = do
  ...
  publicPackages <- createFindEntitiesBySortedFn "config_knowledge_model_public_package_pattern" [("tenant_uuid", U.toString tenantUuid)] [Sort "organization_id" Ascending, Sort "km_id" Ascending, Sort "min_version" Ascending, Sort "max_version" Ascending]
