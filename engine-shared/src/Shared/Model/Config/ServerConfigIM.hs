...
instance FromEnv ServerConfigDatabase where
  applyEnv serverConfig =
    applyEnvVariables
      serverConfig
      [ \c -> applyEnvVariable "DATABASE_CONNECTION_STRING" c.connectionString (\x -> c {connectionString = x})
...
instance FromEnv ServerConfigS3 where
  applyEnv serverConfig =
    applyEnvVariables
      serverConfig
      [ \c -> applyEnvVariable "S3_URL" c.url (\x -> c {url = x} :: ServerConfigS3)
      , \c -> applyEnvVariable "S3_PUBLIC_URL" c.publicUrl (\x -> c {publicUrl = x})
      , \c -> applyEnvVariable "S3_USERNAME" c.username (\x -> c {username = x})
      , \c -> applyEnvVariable "S3_PASSWORD" c.password (\x -> c {password = x})
      , \c -> applyEnvVariable "S3_BUCKET" c.bucket (\x -> c {bucket = x})
      , \c -> applyMaybeEnvVariable "S3_REGION" c.region (\x -> c {region = x})
      ]
...
instance FromEnv ServerConfigAnalytics where
  applyEnv serverConfig =
    applyEnvVariables
      ...
      , \c -> applyEnvVariable "ANALYTICS_EMAIL" c.email (\x -> c {email = x})
      ]
...
instance FromEnv ServerConfigSentry where
  applyEnv serverConfig =
    applyEnvVariables
      ...
      , \c -> applyEnvVariable "SENTRY_DSN" c.dsn (\x -> c {dsn = x})
      ]
