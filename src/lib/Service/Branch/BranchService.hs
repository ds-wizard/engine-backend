...
createBranch context branchDto = do
  ...
  case isValidArtifactId artifactId of
    Nothing -> do
      eitherBranchFromDb <-
        findBranchByArtifactId context artifactId
...
modifyBranch context branchUuid branchDto = do
  ...
  case isValidArtifactId artifactId of
    ...
    Just error -> return . Left $ error
  where
    isAlreadyUsedAndIsNotMine (Right branch) =
      U.toString (branch ^. bUuid) /= branchUuid
