    ...
    describe "Sanatize EditKnowledgeEvent" $ do
      it "Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. chapterIds `shouldBe` Just [chapter2 ^. uuid, chapter1 ^. uuid]
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let kmChapterIds = _chapterUuid <$> [chapter2]
        let edited_e_km1 = e_km1 & chapterIds .~ Just kmChapterIds
    ...
    describe "Sanatize: EditChapterEvent" $ do
      it "Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. questionIds `shouldBe` Just [question2 ^. uuid, question1 ^. uuid]
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let chQuestionIds = _questionUuid <$> [question2]
        let edited_e_km1_ch1 = e_km1_ch1 & questionIds .~ Just chQuestionIds
    ...
    describe "Sanatize: EditQuestionEvent" $ do
      it "Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. answerIds `shouldBe` Just [answerYes1 ^. uuid, answerNo1 ^. uuid]
        resEvent ^. referenceIds `shouldBe` Just [referenceCh2 ^. uuid, referenceCh1 ^. uuid]
        resEvent ^. expertIds `shouldBe` Just [expertLuke ^. uuid, expertDarth ^. uuid]
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        ...
        let edited_1_e_km1_ch1_q2 = e_km1_ch1_q2 & answerIds .~ Just qAnswerIds
        let edited_2_e_km1_ch1_q2 = edited_1_e_km1_ch1_q2 & referenceIds .~ Just qReferenceIds
        let edited_3_e_km1_ch1_q2 = edited_2_e_km1_ch1_q2 & expertIds .~ Just qExpertIds
    ...
    describe "Sanatize: EditAnswerEvent" $ do
      it "Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. followUpIds `shouldBe` Just [followUpQuestion1 ^. uuid]
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let ansFollowUpIds = []
        let edited_e_km1_ch1_q2_aYes1_2 = e_km1_ch1_q2_aYes1_2 & followUpIds .~ Just ansFollowUpIds
    ...
    describe "Sanatize: EditFollowUpQuestionEvent" $ do
      it "Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. answerIds `shouldBe` Just [answerYes4 ^. uuid, answerNo4 ^. uuid]
        resEvent ^. referenceIds `shouldBe` Just []
        resEvent ^. expertIds `shouldBe` Just []
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        ...
        let qReferenceIds = [referenceCh2 ^. uuid]
        let qExpertIds = [expertLuke ^. uuid]
        let edited_1_event = e_km1_ch1_ansYes1_fuq1_ansYes3_fuq2_2 & answerIds .~ Just qAnswerIds
        let edited_2_event = edited_1_event & referenceIds .~ Just qReferenceIds
        let edited_3_event = edited_2_event & expertIds .~ Just qExpertIds
