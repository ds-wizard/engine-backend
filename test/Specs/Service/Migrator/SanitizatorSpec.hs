   ...
   do
    ...
    describe "Sanatize: EditChapterEvent" $ do
      ...
      it "Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let chQuestionUuids = _questionUuid <$> [question2]
    ...
    describe "Sanatize: EditQuestionEvent" $ do
      it "QType - QuestionTypeOptions: Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        resEvent ^. answerUuids `shouldBe` (ChangedValue $ Just [q2_answerYes ^. uuid, q2_answerNo ^. uuid])
      ...
      it "QType - QuestionTypeOptions: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let qAnswerUuids = Just [q2_answerYes ^. uuid]
      ...
      it "QType - QuestionTypeList: Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        let (ConflictState (CorrectorConflict (EditQuestionEvent' resEvent))) = resState ^. migrationState
        resEvent ^. uuid `shouldNotBe` e_km1_ch1_q2 ^. uuid
        let (ChangedValue (Just resAit)) = resEvent ^. answerItemTemplatePlainWithUuids
        resAit ^. questionUuids `shouldBe` [q4_ait1_question6 ^. uuid, q4_ait1_question5 ^. uuid]
      it "QType - QuestionTypeList: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let aitQuestionUuids = [q4_ait1_question6 ^. uuid]
        let aitPlainWithUuids =
              AnswerItemTemplatePlainWithUuids
              { _answerItemTemplatePlainWithUuidsTitle = q4_ait ^. title
              , _answerItemTemplatePlainWithUuidsQuestionUuids = aitQuestionUuids
              }
        let edited_1_e_km1_ch2_q4 =
              e_km1_ch2_q4 & answerItemTemplatePlainWithUuids .~ ChangedValue (Just aitPlainWithUuids)
    ...
    describe "Sanatize: EditQuestionEvent (FollowUps)" $ do
      ...
      it "QType - QuestionTypeOptions: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let qAnswerUuids = Just [q2_aYes_fuq1_aYes_fuq2_answerYes ^. uuid]
        let qReferenceUuids = [referenceCh2 ^. uuid]
        let qExpertUuids = [expertNikola ^. uuid]
      ...
      it "QType - QuestionTypeList: Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        let (ChangedValue (Just resAit)) = resEvent ^. answerItemTemplatePlainWithUuids
        resAit ^. questionUuids `shouldBe`
          [q4_ait1_q6_aYes_fuq4_ait_question2 ^. uuid, q4_ait1_q6_aYes_fuq4_ait_question1 ^. uuid]
      it "QType - QuestionTypeList: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let aitQuestionUuids = [q4_ait1_q6_aYes_fuq4_ait_question2 ^. uuid]
        let aitPlainWithUuids =
              AnswerItemTemplatePlainWithUuids
              { _answerItemTemplatePlainWithUuidsTitle = q4_ait ^. title
              , _answerItemTemplatePlainWithUuidsQuestionUuids = aitQuestionUuids
        ...
        let edited_e_km1_ch2_ansMaybe6_fuq4 =
              e_km1_ch2_ansMaybe6_fuq4 & answerItemTemplatePlainWithUuids .~ ChangedValue (Just aitPlainWithUuids)
        let reqState =
    ...
    describe "Sanatize: EditQuestionEvent (AnswerItemTemplate)" $ do
      ...
      it "QType - QuestionTypeOptions: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let qAnswerUuids = Just [q4_ait1_q6_answerYes ^. uuid]
        let qReferenceUuids = [referenceCh2 ^. uuid]
        let qExpertUuids = [expertNikola ^. uuid]
        let edited_1_event = e_km1_ch2_q4_ait1_q6 & answerUuids .~ ChangedValue qAnswerUuids
        let edited_2_event = edited_1_event & referenceUuids .~ ChangedValue qReferenceUuids
        let edited_3_event = edited_2_event & expertUuids .~ ChangedValue qExpertUuids
      ...
      it "QType - QuestionTypeList: Event - all KM uuids exists, no new added in event" $
       ...
       do
        ...
        let (ChangedValue (Just resAit)) = resEvent ^. answerItemTemplatePlainWithUuids
        resAit ^. questionUuids `shouldBe` [q4_ait1_q5_ait2_question8 ^. uuid, q4_ait1_q5_ait2_question7 ^. uuid]
      it "QType - QuestionTypeList: Event - some KM uuids missing, no new added in event" $
        -- Given:
       do
        let aitQuestionUuids = [q4_ait1_q5_ait2_question8 ^. uuid]
        let aitPlainWithUuids =
              AnswerItemTemplatePlainWithUuids
              { _answerItemTemplatePlainWithUuidsTitle = q4_ait ^. title
              , _answerItemTemplatePlainWithUuidsQuestionUuids = aitQuestionUuids
        ...
        let edited_e_km1_ch2_q4_ait1_q5 =
              e_km1_ch2_q4_ait1_q5 & answerItemTemplatePlainWithUuids .~ ChangedValue (Just aitPlainWithUuids)
        let reqState =
