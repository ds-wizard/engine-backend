...
applyEvent qtnCtn' (ResolveCommentThreadEvent' event) = do
  ...
  mUser <- getUser (event ^. createdBy)
  let updateThread t =
        if (t ^. uuid == event ^. threadUuid) && ((t ^. createdBy ^? _Just . uuid) == event ^. createdBy)
...
applyEvent qtnCtn' (DeleteCommentThreadEvent' event) = do
  qtnCtn <- qtnCtn'
  mUser <- getUser (event ^. createdBy)
  let filterFn t = not ((t ^. uuid == event ^. threadUuid) && ((t ^. createdBy ^? _Just . uuid) == event ^. createdBy))
  let threads =
        case M.lookup (event ^. path) (qtnCtn ^. commentThreadsMap) of
          Nothing -> []
          Just [] -> []
...
applyEvent qtnCtn' (EditCommentEvent' event) = do
  ...
  mUser <- getUser (event ^. createdBy)
  let updateComment c =
        if (c ^. uuid == event ^. commentUuid) && ((c ^. createdBy ^? _Just . uuid) == event ^. createdBy)
...
applyEvent qtnCtn' (DeleteCommentEvent' event) = do
  qtnCtn <- qtnCtn'
  mUser <- getUser (event ^. createdBy)
  let filterFn c = not ((c ^. uuid == event ^. commentUuid) && ((c ^. createdBy ^? _Just . uuid) == event ^. createdBy))
  let updateThread t =
        if t ^. uuid == event ^. threadUuid
