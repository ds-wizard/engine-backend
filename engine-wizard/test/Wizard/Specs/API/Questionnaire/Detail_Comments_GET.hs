module Wizard.Specs.API.Questionnaire.Detail_Comments_GET
  ( detail_comments_GET
  ) where
...
-- ------------------------------------------------------------------------
-- GET /questionnaires/{qtnUuid}/comments
-- ------------------------------------------------------------------------
detail_comments_GET :: AppContext -> SpecWith ((), Application)
detail_comments_GET appContext =
  describe "GET /questionnaires/{qtnUuid}/comments" $ do
    test_200 appContext
    test_403 appContext
    test_404 appContext
...
reqUrlT qtnUuid = BS.pack $ "/questionnaires/" ++ U.toString qtnUuid ++ "/comments"
...
test_200 appContext = do
  create_test_200 "HTTP 200 OK (Owner, Private)" appContext questionnaire1 questionnaire1Ctn [reqAuthHeader]
  create_test_200
...
create_test_200 title appContext qtn qtnCtn authHeader =
  it title $
   ...
   do
    ...
    let expDto = toCommentListDTO (qtnCtn ^. commentThreadsMap)
...
test_404 appContext =
  createNotFoundTest'
    reqMethod
    "/questionnaires/f08ead5f-746d-411b-aee6-77ea3d24016a/comments"
