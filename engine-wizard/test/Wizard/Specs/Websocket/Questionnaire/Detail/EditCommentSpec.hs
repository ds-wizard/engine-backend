...
test200 appContext =
  it "WS 200 OK" $
   ...
   do
    ...
    -- WHEN:
    write_EditComment c1 (toEventChangeDTO ece_rQ1_t1_1' samplePhasesAnsweredIndication)
    -- THEN:
    read_EditComment c1 (toEventDTO ece_rQ1_t1_1' (Just userAlbert))
    read_EditComment c2 (toEventDTO ece_rQ1_t1_1' (Just userAlbert))
    read_EditComment c3 (toEventDTO ece_rQ1_t1_1' (Just userAlbert))
